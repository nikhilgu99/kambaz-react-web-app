...
export default function WorkingWithArrays() {
  ...
  return (
    <div id="wd-working-with-arrays">
      ...
      <h3>Updating an Item's Completion in an Array</h3>
      <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-primary float-end">
        Update Completion</a>
      <FormControl defaultValue={todo.id} className="w-25 float-start"
        onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
      <Form.Check className="float-start" id="wd-array-completion" type="checkbox"
      isValid={todo.completed} label="Completed" onChange={(e) =>
